...
using UnityEngine;
public class get_arduino : MonoBehaviour
{
    private SerialPort arduinoStream;
    public string port;
    private Thread readThread; // 宣告執行緒
    public string readMessage;
    public float m_FieldOfView, m_Focus;
   // public int change = 0;
    bool isNewMessage;
    ...
    void Update () {
        if (isNewMessage) {
            if (float.Parse(readMessage) >= 1030)
            {
                m_Focus = ((float.Parse(readMessage)-1030) / 3.4f) + 1;
            }
            else
            {
                m_FieldOfView = (int)(float.Parse(readMessage) / 6) + 1;
            }
